...
module.exports.post = function (req, res) {
	logger.trace('Github event received');
	...
	Repo.findOne({ repoId: req.body.repository.id }, function (err, repo) {
		...
		logger.trace('Repo found. Creating jobs for commits...');

		async.forEach(req.body.commits, function (commit) {

			logger.trace('Creating job for commit', commit);
			...
			Job.create({
				...
				logger.trace('Job published to queue', commit);
